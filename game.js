...
const shipImgs = {};
Promise.all(Object.entries(ships).map(async ([key, src]) => {
    shipImgs[key] = await loadImage(src);
})).then(() => {
    console.log("Todas las imágenes de naves cargadas.");
});

const enemyImg = await loadImage("img/enemy.png");
const bossImgs = await Promise.all(["img/boss.png","img/boss1.png","img/boss2.png","img/boss3.png","img/boss4.png","img/boss5.png"]
    .map(loadImage));
...
// Función genérica para crear un beep
...
const shootSound = () => beep(800, 0.08, 0.2, "triangle");
const enemyDie = () => beep(200, 0.15, 0.4, "sawtooth");
const bossHit = () => beep(100, 0.3, 0.6, "square");
const playerHit = () => beep(150, 0.4, 0.5, "sine");
const explosionSound = () => beep(80, 0.4, 0.8, "sawtooth");
...
// Manejador de eventos para teclas presionadas
window.addEventListener("keydown", e => {
    ...
    if (e.code === "Space") e.preventDefault(); // Evita el scroll
});

// Manejador de eventos para teclas liberadas
window.addEventListener("keyup", e => keys[e.code] = false);

// Disparo
let canShoot = true;

// Evento de disparo al presionar la barra espaciadora
window.addEventListener("keydown", e => {
...
function update() {
    ...
    function handleEnemyHit(bullet, bulletIndex, enemyIndex) {
        createExplosion(enemies[enemyIndex].x + 20, enemies[enemyIndex].y + 20);
        enemyDie();
    ...
    function handleBossHit(bullet, bulletIndex) {
        currentBoss.life -= 8 + (doubleShot ? 4 : 0);
        bullets.splice(bulletIndex, 1);
        bossHit();
        createParticles(bullet.x, bullet.y, "cyan", 10);
    ...
    [...enemyBullets, ...bossBullets].forEach(b => {
        if (player.invulnerable === 0 && coll(b, player)) {
            handlePlayerDamage(b);
        }
    });

    function handlePlayerDamage(bullet) {
        lives--;
        playerHit();
